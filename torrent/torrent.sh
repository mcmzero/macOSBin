    ...
    case $1 in
        ...
        ls*|ta*)
            if [ "$(hostname -s)" == "rasPi" ]; then
                ...
                sqlite3 /usr/local/torrent/magnet.db "SELECT datetime(time, 'unixepoch', 'localtime'),magnet,name FROM magnetList ORDER BY time DESC  LIMIT 15;" -separator ' '
            else
                #ssh pi@rpi "tail ${magnetListFile}_* | tail"
                ssh pi@rpi "sqlite3 /usr/local/torrent/magnet.db \"SELECT datetime(time, 'unixepoch', 'localtime'),magnet,name FROM magnetList ORDER BY time DESC  LIMIT 15;\" -separator ' '"
            fi
        ;;
        -f)
