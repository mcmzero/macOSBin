...
function get_target_name() {
	...
	target_name=$(echo -n "$target_name"|sed -e 's/....[pP]-[nN][eE][xX][tT]//' -e 's/....[pP]-[wW][iI][tT][hH]//')
	target_name=$(echo -n "$target_name"|sed -e 's/....[pP]-[dD][wW][bB][hH]//' -e 's/....[pP]-[cC][iI][nN][eE][bB][uU][sS]//')
	target_name=$(echo -n "$target_name"|sed -e 's/\([0-9][0-9][0-9][0-9][0-9][0-9]\)-.*.m/\1.m/' -e 's/\([0-9][0-9][0-9][0-9][0-9][0-9]\)-.*.a/\1.a/')
...
function get_target_path_name() {
	...
	target_path=$(echo $target_path | sed -e 's/.*드라마 스페셜.*/KBS 드라마 스페셜/')
	target_path=$(echo $target_path | sed -e 's/.*드라마 스테이지.*/드라마 스테이지/')
	target_path=$(echo $target_path | sed -e 's/.*TV정보쇼.*/TV정보쇼/')
	target_path=$(echo $target_path | sed -e 's/.*한편으로 정주행.*/몰아보기/')
	target_path=$(echo $target_path | sed -e 's/.*의문의 일승.*/의문의 일승/')
	target_path=$(echo $target_path | sed -e 's/.*몰아보기.*/몰아보기/')
	target_path=$(echo $target_path | sed -e 's/.*한편으로 정주행/몰아보기/')
	target_path=$(echo $target_path | sed -e 's/.*몰아보기/몰아보기/')
	target_path=$(echo $target_path | sed -e 's/.*복면가왕.*/복면가왕/')
	target_path=$(echo $target_path | sed -e 's/.*신혼일기.*/신혼일기/')
	target_path=$(echo $target_path | sed -e 's/.*알쓸신잡.*/알쓸신잡/')
	target_path=$(echo $target_path | sed -e 's/.*시골경찰.*/시골경찰/')
	target_path=$(echo $target_path | sed -e 's/.*식신로드.*/식신로드/')
	target_path=$(echo $target_path | sed -e 's/.*겟잇뷰티.*/겟잇뷰티/')
	target_path=$(echo $target_path | sed -e 's/.*응답하라.*/응답하라/')
	target_path=$(echo $target_path | sed -e 's/.*개밥 주는 남자.*/개밥 주는 남자/')
	target_path=$(echo $target_path | sed -e 's/.*내 딸의 남자들.*/내 딸의 남자들/')
	target_path=$(echo $target_path | sed -e 's/.*수업을 바꿔라.*/수업을 바꿔라/')
	target_path=$(echo $target_path | sed -e 's/.*맛있을 지도.*/맛있을 지도/')
	target_path=$(echo $target_path | sed -e 's/.*소사이어티 게임.*/소사이어티 게임/')
	target_path=$(echo $target_path | sed -e 's/.*SNL 코리아.*/SNL 코리아/')
	target_path=$(echo $target_path | sed -e 's/.*별거가 별거냐.*/별거가 별거냐/')
	target_path=$(echo $target_path | sed -e 's/.*팬텀싱어.*/팬텀싱어/')
	target_path=$(echo $target_path | sed -e 's/.*판타스틱 듀오.*/판타스틱 듀오/')
	target_path=$(echo $target_path | sed -e 's/.*유닛.*/더 유닛/')
	target_path=$(echo $target_path | sed -e 's/.*집밥 백선생.*/집밥 백선생/')
